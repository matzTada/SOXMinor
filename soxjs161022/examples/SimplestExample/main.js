var boshService = "http://osmanthus:7070/http-bind/";
var xmppServer = "osmanthus";
...
window.onload = function() {
	...
	//SOXサーバに接続したらdeviceNameを取得して、メタデータの表示、サブスクライブをするプログラムになっています。
	soxEventListener.connected = function(soxEvent) {
		console.log("[main.js] Connected " + soxEvent.soxClient);
		status("Connected: " + soxEvent.soxClient);

		//deviceNameデバイスのインスタンスを作ります。ここのノード名を他のにすると、そのノード名のデータが表示されるようになります。
		var device = new Device(deviceName);

		//deviceNameデバイスの情報を表示してみます
		//soxEventListener.resolved = function(soxEvent) 部分が呼ばれます。
		client.resolveDevice(device);

		/* deviceNameデバイスをサブスクライブします */
